...
def iteration_solve_init(G,source,x,x_old,epsilon,node_max,iC,method):
    cnt = 0
    while ((abs(x_old[3]-x[3]))>epsilon and (cnt<100) ):
        x_old = x
        node = node_max 
...
def iteration_solve(G,source,x,x_old_iter,x_old_time,VA,VB,epsilon,node_max,iC,method,h):
    cnt = 0
    x_old_iter[:] = 100
    while ((abs(x_old_iter[3]-x[3])>epsilon) and (cnt<100) ):
        x_old_iter = x
        node = node_max
...
# update source for TR
# def update_C(x,iC,vC,h):
#     # Cgs N1
#     iC[0] = 2e-14*(x[4] - x[0] - vC[0]) - iC[0]
#     vC[0] = x[4] - x[0]
#     vC[1] = x[4] - x[1]
#     iC[1] = 2e-14*vC[1] - iC[1]
