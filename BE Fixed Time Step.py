...
def read_netlist(netlist,total_list,node_max,G,source,x,x_old_time,VA,VB,iC,method,h,update_iC):
    ...
    for line in total_list:
        ...
        if 'VA' == component[0]: 
            ...
            #     # Cgs
            #     if 1 == update_iC:
            #         iC[C_index] = (2e-14/h)*(x_old_time[int(component[1])-1]-x_old_time[int(component[2])-1])-iC[C_index] 
            ...
            #     # Cgd
            #     if 1 == update_iC:
            #         iC[C_index] = (2e-14/h)*(x_old_time[int(component[1])-1]-x_old_time[int(component[3])-1])-iC[C_index]
            ...
            #     # Cgd
            #     if 1 == update_iC:
            #         iC[C_index] = (2e-14/h)*(x_old_time[int(component[1])-1]-x_old_time[int(component[3])-1,0])-iC[C_index]
            #     G,source = C_TR(G, source, int(component[1])-1, int(component[3])-1, 2e-14/h, x_old_time[int(component[1])-1]-x_old_time[int(component[3])-1], iC[C_index])
            #     C_index += 1
        elif 'R' == component[0][0]:
            G = R_stamp(G, 1/(float(component[3])*unit_conversion(component[4][0])), int(component[1])-1, int(component[2])-1)
            ...
            if 'BE' == method:
                ...
                if 1 == update_iC:
                    iC[C_index] = (2*float(component[3])*unit_conversion(component[4][0])/h)*(x_old_time[int(component[1])-1]-x_old_time[int(component[2])-1])-iC[C_index]
