...
def LTE(x_old,x,method,Bu,Bl,h,h_old,hmin):
    ...
    B = 0.5 * ( Bu + Bl )
    if 'BE' == method:
        LTE = 2*abs(((h**2)/(h+h_old[0]))*((x[jj]-x_old[jj,0])/h-(x_old[jj,0]-x_old[jj,1])/h_old[0]))
...
T = 0.0
T_plot = np.ndarray(1000,dtype=float)
solution = np.ndarray(1000,dtype=float)
T_plot[:] = 0
solution[:] = 0
...
plt.plot(T_plot[0:ii+1], solution[0:ii+1], label=method + ' w. dynamic step')
